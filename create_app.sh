...
HTML_PLAYER=$APP_DIR/player.html
HTML_PLAY=$APP_DIR/js/embedded-player.js
...
#cat >> "$RED5_PROPERTIES_FILE" << 'EOF'
#stunServerURI=${STUN_URI}
#turnServerUsername=${TURN_USERNAME}
#turnServerCredential=${TURN_PASSWORD}
#EOF

echo "stunServerURI=turn:ovh36.antmedia.io:3478" >> $RED5_PROPERTIES_FILE 
...
echo "turnServerUsername=$TURN_USERNAME" >> $RED5_PROPERTIES_FILE
echo "turnServerCredential=$TURN_PASSWORD" >> $RED5_PROPERTIES_FILE

sed -i "/this.iceServers/s/19302\" } \]'/19302\"},{\"urls\":\"$TURN_URL\",\"username\":\"$TURN_USERNAME\",\"credential\":\"$TURN_PASSWORD\"}]'/" $HTML_PLAY 
